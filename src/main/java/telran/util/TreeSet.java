...
public class TreeSet<T> implements SortedSet<T> {
    ...
    @Override
    public T last() {
        TreeSetIterator iterator = new TreeSetIterator();
        T res = null;
    ...
    public T floor(T key) {
        ...
        while (iterator.hasNext()) {
            ...
            if (comparator.compare(nextValue, key) <= 0) {
                res = nextValue;
            } else {
                break;
            }
    ...
    public SortedSet<T> subSet(T keyFrom, T keyTo) {
        if (!insideTreeSet(keyFrom) || !insideTreeSet(keyTo)) {
            throw new IllegalArgumentException();
        }
        TreeSet <T> sub = new TreeSet<>();
        TreeSetIterator iterator = new TreeSetIterator();
        while(iterator.hasNext()) {
            T nextValue = iterator.next();
            if(insideRange(keyFrom, keyTo, nextValue)) {
                sub.add(nextValue);
            }
        }
        return sub;
    ...
    private boolean insideTreeSet(T pattern) {
        return (comparator.compare(first(), pattern) < 0) && (comparator.compare(last(), pattern) > 0);
    }

    private boolean insideRange(T keyFrom, T keyTo, T pattern) {
        return (comparator.compare(keyFrom, pattern) <= 0) && (comparator.compare(keyTo, pattern) > 0);
    }
}
